...
<!-- =========================================================================== 
	...
	schema - just the restricted one defined by the profile. Please provide any 
	comments or feedback on this validator to Arne Broering (arneb@uni-muenster.de) 
	=========================================================================== 
	REFERENCES: =========================================================================== 
	[1]. http://www.schematron.com/iso/Schematron.html [2]. http://www.opengeospatial.org/standards/sensorml 
	...
	<pattern id="SystemValidation">
		...
		<rule context="//sml:position/swe:Position/swe:location">
			<assert test="count(swe:Vector/swe:coordinate/swe:Quantity) > 1">Error: 'swe:location' has to specify at least 2
				'swe:Vector/swe:coordinate/swe:Quantity' elements.</assert>
		</rule>
		...
		<rule context="//sml:System/sml:components/sml:ComponentList/sml:component">
			...
			<assert
				...
				'xlink:href' OR child 'sml:Component'</assert>
		</rule>
	</pattern>
	...
	<pattern id="ComponentValidation">
		...
		<rule context="//sml:Component/sml:classification">
			<assert
				...
				'urn:ogc:def:classifier:OGC:1.0:sensorType'.</assert>
		</rule>
	</pattern>
	...
	<pattern id="GeneralValidation">
		...
		<rule
			...
			<assert test="string-length(@definition) > 0">Error: 'definition' attribute has to be present
				and its value has to be > 0.</assert>
		</rule>
		...
		<rule context="//sml:identification">
			<assert
				...
				'urn:ogc:def:identifier:OGC:1.0:uniqueID'.</assert>
			<assert
				...
				'urn:ogc:def:identifier:OGC:1.0:longName'.</assert>
			<assert
				...
				'urn:ogc:def:identifier:OGC:1.0:shortName'.</assert>
		</rule>
		...
		<rule context="//sml:Component/sml:identification">
			<assert
				...
				'urn:ogc:def:identifier:OGC:parentSystemUniqueID'.</assert>
		</rule>
		...
		<rule context="//sml:classification">
			<assert
				test="count(sml:ClassifierList/sml:classifier/sml:Term[@definition = 'urn:ogc:def:classifier:OGC:1.0:sensorType']) >= 1">Error: one classifier for the sensorType classification scheme has to be given!</assert>
			<assert
				test="count(sml:ClassifierList/sml:classifier/sml:Term[@definition = 'urn:ogc:def:classifier:OGC:1.0:application']) >= 1">Error: one classifier for the intendedApplication classification scheme has to be given!</assert>
		</rule>
		...
		<!-- Each "classifier/Term" element contained in the "ClassifierList" must 
			...
			identifier. -->
		<rule
			context="//sml:classifier/sml:Term">
			<assert test="string-length(@definition) > 0">Error: 'definition' attribute has to be present
				and its value has to be > 0.</assert>
		</rule>
