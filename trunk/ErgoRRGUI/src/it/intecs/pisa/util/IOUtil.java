...
public class IOUtil {
    ...
    public static String[] listDir(File schemaDir, String removeStartPath) {
        ...
        for (int u=0; u<files.size();u++) {
            String path=(String)files.get(u);
    ...
    private static Vector listSubDir(File schemaDir) {
        File subDir;
        File[] listingFiles;
        Vector files;
        ...
        {
            for (int u=0; u<listingFiles.length;u++) {
                 File f=listingFiles[u]; 
                if (f.isDirectory()) {
