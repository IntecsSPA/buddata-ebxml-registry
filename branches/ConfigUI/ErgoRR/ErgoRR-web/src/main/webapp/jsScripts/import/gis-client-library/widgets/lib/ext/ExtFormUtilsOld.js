                  ...
                  var formPanelDropTarget = new Ext.dd.DropTarget(formPanelDropTargetEl, {
                        ...
                        notifyDrop  : function(ddSource, e, data){
                            ...
                            for(var i=0; i<fields.length; i++ ){
                                ...
                                formPanel.getForm().findField(fields[i]).setValue(selectedRecord.data[formPanel.getForm().findField(fields[i]).storeAttribute]);
                            }
                               
                            //alert(selectedRecord.data.ProductIdentifier);
                            //formPanel.getForm().findField("input1").setValue(selectedRecord.data.ProductIdentifier);
                            // Load the record into the form
                           // this.formsArray[index].getForm().loadRecord(selectedRecord);
                                ...
                                notifyDrop  : function(ddSource, e, data){
                                    ...
                                    for(var u=0; u<fieldSetsInputs[this.ddGroup].length; u++ ){
                                       if(fieldSetsInputValues[this.ddGroup][u])
                                        formPanel.getForm().findField(fieldSetsInputs[this.ddGroup][u]).setValue(fieldSetsInputValues[this.ddGroup][u]);
                                       else
                                        formPanel.getForm().findField(fieldSetsInputs[this.ddGroup][u]).setValue(selectedRecord.data[formPanel.getForm().findField(fieldSetsInputs[this.ddGroup][u]).storeAttribute]);
                                    }
...
function createExjFormByElement(title, formDataElement, numCols, localizationObj){
   ...
   var groupFormElements;
    ...
    var formElement=new XmlElement(formDataElement);
    groupFormElements= formElement.selectNodes("gis:group");
    alert(groupFormElements.length);
